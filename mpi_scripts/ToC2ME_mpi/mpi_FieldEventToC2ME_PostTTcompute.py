...
Run as: export OMP_NUM_THREADS=2; export MKL_NUM_THREADS=2; export NUMBA_NUM_THREADS=2; mpiexec -n 4 python mpi_Kirchhoff_PostTTcompute_ToC2ME.py
"""
...
def run():
    ...
    # LOAD FIELD DATA
    event_folder = "/home/birniece/Documents/data/ToC2ME/Omar/EV1115_173305/"
    event_files = sorted(glob.glob(event_folder + '*.mseed'),
                         key=lambda x: int(os.path.basename(x).split('/')[-1][:-6]))
    trace_data = np.zeros([69, 12000])
    for i, evf in enumerate(event_files):
        st = read(evf)
        trace_data[i] = st[0].data
    ...
    if rank == 0:
        ...
        print(f'Adjoint elapsed time: {toca-tica} s')
        print(f'Imaging elapsed time: {toci-tici} s')
